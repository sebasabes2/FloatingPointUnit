import floatingPointUnit.LeftNormalizer
import chisel3._
import chiseltest._
import org.scalatest.flatspec.AnyFlatSpec

class LeftNormalizerTest extends AnyFlatSpec with ChiselScalatestTester {
  "LeftNormalizer" should "not normalize" in {
    test(new LeftNormalizer(8, 24)) { dut =>
  ...
  "LeftNormalizer" should "normalize left" in {
    test(new LeftNormalizer(8, 24)) { dut =>
      dut.io.input.sign.poke("x1".U)
      dut.io.input.exponent.poke("x80".U)
  ...
  "LeftNormalizer" should "pass ieee754-test-suite/Add-Cancellation.fptest:20" in {
    test(new LeftNormalizer(8, 24)) { dut =>
      dut.io.input.sign.poke("x1".U)
