...
class Rounder(exponentWidth: Int, significandWidth: Int) extends Module {
  ...
  switch (io.roundingMode) {
    ...
    is (2.U) { rnd := negativeInf }
    is (3.U) { rnd := positiveInf }
    is (4.U) { rnd := nearestAway }
  }

  val rounded = io.input.significand +& rnd

  io.output := io.input
  io.output.significand := rounded
  io.output.guard := 0.U
  io.output.round := 0.U
  io.output.sticky := 0.U
  io.output.denormal := io.input.denormal && !rounded(significandWidth - 1)
